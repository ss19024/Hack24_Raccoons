...
namespace UnityEngine.Rokid.XR.ARFoundation
{
    /// <summary>
    ...
    public static class ARPlaneMeshGenerators
    {
        ...
        public static bool GenerateGradientMesh(Mesh mesh, Pose pose, Vector2[] convexPolygon, float gradientRadius, float areaTolerance = 1e-6f)
        {
            if (mesh == null)
                throw new ArgumentNullException("mesh");
            ...
            s_Vertices.Clear();
            var center = Vector3.zero;
            float scale = Mathf.Max(gradientRadius, 0);
            foreach (var point2 in convexPolygon)
            {
                var point3 = new Vector3(point2.x, 0, point2.y);
                var scalePoint = new Vector3(point2.x * scale, 0, point2.y * scale);
                center += point3;
                center += scalePoint;
                s_Vertices.Add(point3);
                s_Vertices.Add(scalePoint);
            }
            center /= convexPolygon.Length * 2;
            s_Vertices.Add(center);

            // If the polygon is too small or degenerate, no mesh is created.
            if (!GenerateGradientIndices(s_Indices, s_Vertices, areaTolerance))
                return false;
            ...
            // UVs
            GenerateUvs(m_Uvs, pose, s_Vertices);
            for (int i = 0; i < m_Uvs.Count - 1; i++)
            {
                if (i % 2 == 0)
                {
                    m_Uvs[i] = new Vector3(m_Uvs[i].x, m_Uvs[i].y, 1);
                }
                else
                {
                    m_Uvs[i] = new Vector3(m_Uvs[i].x, m_Uvs[i].y, 0);
                }
            }
        ...
        /// /// <param name="pose">The session-space pose of the mesh.</param>
        /// <param name="vertices">The vertices of the plane's boundary, in plane-space.</param>
        public static void GenerateUvs(List<Vector3> Uvs, Pose pose, List<Vector3> vertices)
        {
            // Get the twist rotation about the plane's normal, then apply
            // its inverse to the rotation to produce the "untwisted" rotation.
            // This is similar to Swing-Twist Decomposition.
            ...
            foreach (var vertex in vertices)
            {
                ...
                var uv = new Vector3(
                    Vector3.Dot(vertexInSessionSpace, sessionSpaceRight),
                    Vector3.Dot(vertexInSessionSpace, sessionSpaceForward), 0);
        ...
        public static bool GenerateGradientIndices(List<int> indices, List<Vector3> convexPolygon, float areaTolerance = 1e-6f)
        {
            indices.Clear();
            ...
            for (int i = 0; i < numBoundaryVertices; ++i)
            {
                ...
                if (i % 2 == 0)
                {
                    indices.Add((idx0 + 3) % numBoundaryVertices);
                    indices.Add((idx0 + 1) % numBoundaryVertices);
                    indices.Add(idx0);

                    indices.Add((idx0 + 2) % numBoundaryVertices);
                    indices.Add((idx0 + 3) % numBoundaryVertices);
                    indices.Add(idx0);
                }
                else
        ...
        // Caches to avoid reallotting Lists during calculations
        static List<int> s_Indices = new List<int>();

        static List<Vector2> s_Uvs = new List<Vector2>();

        private static List<Vector3> m_Uvs = new List<Vector3>();
